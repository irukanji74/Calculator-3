...
public class ExpressionHolder2 {
	...
	public void setExpression(String symbol) {
		if ((firstNum.length() == 0
				|| (!Calculations.isMatchOperator(firstNum.substring(firstNum.length() - 1, firstNum.length()))))
		...
		if(firstNum.length() != 0 && Calculations.isMatchOperator(firstNum.substring(firstNum.length() - 1, firstNum.length()))
				&&  (!Calculations.isMatchOperator(symbol))) {
		...
		if ((Calculations.isMatchOperator(symbol) && secondNum.length() == 0) && Calculations.isMatchNumber(firstNum.toString())) {
			firstNum.append(symbol);
			return;
		} 
